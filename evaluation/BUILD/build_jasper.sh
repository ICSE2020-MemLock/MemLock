...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if [ -d "$(dirname ${BIN_PATH})/jasper/SRC_MemLock/build" ]; then
		rm -rf $(dirname ${BIN_PATH})/jasper/SRC_MemLock/build
	fi
    ...
    mkdir $(dirname ${BIN_PATH})/jasper/SRC_MemLock/build
	...
	if [ -d "$(dirname ${BIN_PATH})/jasper/SRC_AFL/build"  ]; then
        rm -rf $(dirname ${BIN_PATH})/jasper/SRC_AFL/build
    fi
    export CC=${ROOT_DIR}/tool/AFL-2.52b/build/bin/afl-clang-fast
    export CXX=${ROOT_DIR}/tool/AFL-2.52b/build/bin/afl-clang-fast++
    export CFLAGS="-g -O0 -fsanitize=address"
    export CXXFLAGS="-g -O0 -fsanitize=address"
    mkdir $(dirname ${BIN_PATH})/jasper/SRC_AFL/build
