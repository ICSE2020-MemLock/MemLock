...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if ! [ -d "${ROOT_DIR}/evaluation/BUILD/nm/SRC_MemLock/build/bin" ]; then
	...
	${ROOT_DIR}/tool/MemLock/build/bin/memlock-stack-fuzz -i ${ROOT_DIR}/evaluation/BUILD/nm/SEED/ -o out_MemLock$i -m none -d --  ${ROOT_DIR}/evaluation/BUILD/cxxfilt/SRC_MemLock/build/bin/nm -C @@
fi
