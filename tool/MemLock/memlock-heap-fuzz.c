/*
   ...
   MemLock design by wcventure <wcventure@126.com>
   
   Copyright 2018, 2019, 2020 SZU Inc. All rights reserved.
...
EXP_ST u64* perf_bits;                /* PERF - SHM with 2nd (perf) map   */
EXP_ST u64 max_counts[PERF_SIZE];     /* PERF - keeps track of max value  */
EXP_ST u64 staleness[PERF_SIZE];      /* PERF - the staleness max values */
...
struct queue_entry {
  ...
  u64 exec_us,                        /* Execution time (us)              */
      ...
      sizeScore,                      /* Added: sizeScore (wcventure)     */
      depth;                          /* Path depth                       */
...
static u64 sizeScore_cur = 0;         /* Added: cur sizeScore (wcventure) */
...
EXP_ST void setup_shm(void) {
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE + (PERF_SIZE * sizeof(u64)), IPC_CREAT | IPC_EXCL | 0600);
  ...
  if (max_ct_fuzzing) perf_bits = (u64 *) (trace_bits + MAP_SIZE);
...
EXP_ST void setup_max_counts() {
  memset(max_counts, 0, PERF_SIZE * sizeof(u64));
}
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (max_ct_fuzzing) memset(perf_bits, 0, PERF_SIZE * sizeof(u64));
  MEM_BARRIER();
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    ReadMemStatus(&MaxMalloc);
    sizeScore_cur = MaxMalloc;
    if (sizeScore_cur > sizeScore_max)
      sizeScore_max = sizeScore_cur;
    ...
    if (q->exec_cksum != cksum) {
      ...
      if (q->exec_cksum) {
        ...
        if (max_ct_fuzzing) 
          q->perf_cksum = hash32(perf_bits, PERF_SIZE*sizeof(u64), HASH_CONST); 
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    if (res == crash_mode || res == FAULT_NOBITS)
      SAYF(cGRA "    len = %u, map size = %u, exec speed = %llu us, sizeScore = %llu\n" cRST, 
           q->len, q->bitmap_size, q->exec_us, q->sizeScore);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (max_ct_fuzzing) 
      queue_top->perf_cksum = hash32(perf_bits, PERF_SIZE*sizeof(u64), HASH_CONST); 
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%lld", sizeScore_max);

  SAYF (bSTG bV bSTOP "   Memory peak : " cRST "%s%-22s " bSTG bV "\n", cRST, tmp);
...
static u8 trim_case(char** argv, struct queue_entry* q, u8* in_buf) {
  ...
  static u64 clean_perf[PERF_SIZE];
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, TRIM_MIN_BYTES)) {
    ...
    while (remove_pos < q->len) {
      ...
      if (max_ct_fuzzing)
        perf_cksum = hash32(perf_bits, PERF_SIZE*sizeof(u64), HASH_CONST);
      ...
      if ((exec_cksum == q->exec_cksum) && 
        ...
        if (!needs_write) {
          ...
          if (max_ct_fuzzing) memcpy(clean_perf, perf_bits, PERF_SIZE*sizeof(u64)); 
  ...
  if (needs_write) {
    ...
    if (max_ct_fuzzing) memcpy(perf_bits, clean_perf, PERF_SIZE*sizeof(u64));
    update_bitmap_score(q);
